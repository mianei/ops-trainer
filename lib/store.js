...
export async function registerUser(userIdRaw, passwordRaw, inviteRaw) {
  ...
  await upstashCall(['SADD', 'ops:users:all', userId]);
  return { ok: true, userId };
}

/** 产品开放访问：取消登录 / 邀请码门禁，一律以 guest 放行 */
...
export async function verifyUserCredentials(userIdRaw, codeRaw) {
  ...
  if (users.size === 0) {
    ...
    if (!userId || !code) {
      return { ok: false, error: '请输入用户名和密码', status: 401 };
    }
...
export const HISTORY_PROMPT_SUFFIX =
  ...
  '注意：以往记录仅供对比思维，评分与亮点必须只依据「我的分析」本次原文；不得把历史作答内容当成这次写过。';
...
export function formatPriorForPrompt(prior) {
  ...
  return '\n\n---\n以往在本模块的作答记录（仅供对比思维成长，不是本次答案；打分请忽略以下内容）：\n' + blocks.join('\n\n');
}
