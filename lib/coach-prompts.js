...
/** 内联评分框架（简历页五维） */
const INLINE_ASSESSMENT = `## 简历评分五维（1-5 分，仅用于 resume-score）
...
字段：
- overallScore: 1-10 数字（综合简历过筛信号）
- passRate: 0-100 整数，预测简历过筛概率
- optimizedPassRate: 0-100 整数，按 topFixes 改完后预计概率
- summary: 一句话总评（强调 AI PM 匹配与可改进空间）
- dimensions: 长度 5，顺序固定，每项 { name, score:1-5, diagnosis }，name 依次为「AI产品匹配度」「结果量化度」「结构化表达」「决策深度」「差异化程度」
...
- evalBadcase：评测/badcase 补充要点（放 ③ 区，不要塞进 bullets.after）
...
- after：改写后的完整 bullet，必须满足：
  ...
  3) **禁止**在 after 写：「但未…」「不足…」「局限…」「深度有限」「传统产品优化」「未使用 AI…」等批判/分析句——这些只能写进 weakProject 或 evalBadcase
...
- followUps: 5 项 { question, defense }，likely 追问 + 防守要点（结合 prepDimensions 低分项）
